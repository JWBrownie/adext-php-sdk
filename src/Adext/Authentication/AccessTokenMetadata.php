...
class AccessTokenMetadata
{
    ...
    public function __construct(array $metadata)
    {
        if (!isset($metadata['data'])) {
            throw new AdextSDKException('Unexpected debug token response data.', 401);
        }
     ...
     *
     * @see https://developers.adext.com/docs/adex-login/access-tokens#debug
     *
     * @return \DateTime|null
     */
    ...
    public function validateAppId($appId)
    {
        if ($this->getAppId() !== $appId) {
            throw new AdextSDKException('Access token metadata contains unexpected app ID.', 401);
        }
    }
    ...
    public function validateUserId($userId)
    {
        if ($this->getUserId() !== $userId) {
            throw new AdextSDKException('Access token metadata contains unexpected user ID.', 401);
        }
    }
    ...
    public function validateExpiration()
    {
        ...
        if ($this->getExpiresAt()->getTimestamp() < time()) {
            throw new AdextSDKException('Inspection of access token metadata shows that the access token has expired.', 401);
        }
    }
